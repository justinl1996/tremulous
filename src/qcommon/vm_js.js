var LibraryVM = {
	...
	$VM: {
		...
		vm_t: {            
            ...
            "instructionPointers": 300,
            "jumpTableTargets": 340,
            "name": 8,
            "numJumpTableTargets": 344,
            "numSymbols": 320,
            "programStack": 0,
            "searchPath": 264,
            "stackBottom": 316,
		...
		CompileModule: function (name, instructionCount, codeBase, dataBase) {
			...
			if (fs_game === 'cpma' && name === 'cgame') {
				...
				EmitStatement('\t\tvar modelName = Module.UTF8ToString(' + state.dataBase + ' + {{{ makeGetValue("image", "stackOnEntry + 8", "i32") }}});');
				...
				EmitStatement('\t\t\t(modelName, ' + state.dataBase + ' + STACKTOP);');
	...
	VM_Compile: function (vmp, headerp) {
		...
		var name = Module.UTF8ToString(vmp + VM.vm_t.name);
